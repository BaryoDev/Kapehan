...
| Type declarations | 7 `.d.ts` covering the 9 code subpaths, generated by `scripts/generators/types.mjs`. The remaining 4 exports need none: `kapehan.css`, the `icons/*` and `doodles/*` SVG wildcards, and `palettes.json`, which `resolveJsonModule` types from the file itself. |
...
The package surface is complete **in this repo**. It is not all installable yet: npm still
serves 0.2.0, so `npm i kapehan` gives you the icons, the web component and `kapehan.css`.
The palettes, the doodles, the React components and the type declarations are on `master`
and reach npm when 0.3.0 is published.
...
kapehan/
  ...
  package.json exports    13 subpaths; the 8 code ones carry types first       [SHIPPED]
  types                   7 .d.ts, generated from the sources                  [SHIPPED]
